...
interface Props { block: Block; index: number; }

export function NumberedBlock({ block, index }: Props) {
  ...
  return (
    <div className="flex items-start gap-2" style={{ paddingLeft: `${indent * 24}px` }}>
      <span className="shrink-0 mt-0.5 text-[var(--text-tertiary)] select-none w-6 text-right leading-7 text-sm tabular-nums">
        {index}.
      </span>
      <div
        ref={contentRef}
